...
def profile():
    ...
    uid = session[CURR_USER_KEY]
    user = User.query.get_or_404(uid)
    form = UserAddForm(obj=user)

    if form.validate_on_submit():
        user.image_url = form.image_url.data if form.image_url.data else None
        user.header_image_url = form.header_image_url.data if form.header_image_url.data else None
...
def homepage():
    ...
    if g.user:
        user = User.query.get(g.user.id)
        following_users = [user.id for user in user.following]
